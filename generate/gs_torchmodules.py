#change Gretel Synthetics torch_modules.py with this script
...
class Generator(torch.nn.Module):
    ...
    def forward(
        ...
        if self.attribute_gen is not None:
            ...
            #apply an output mask in the generator?
...
class Discriminator(torch.nn.Module):
    ...
    def __init__(self, input_dim: int, num_layers: int = 5, num_units: int = 200,
                 features: bool = False, max_seq_len: int = 40,total_attributes: int = 12):
        ...
        if self.features:
            seq.append(MaskingLayer(mask_value=1,discriminator=True,
                                    max_seq_len=self.max_seq_len,total_attributes=self.total_attributes))
...
class MaskingLayer(torch.nn.Module):
    def __init__(self, mask_value: int = 1 , discriminator: bool = True,
        ...
        self.discriminator = discriminator          #indicates discriminator/generator
        self.max_seq_len = max_seq_len              #needed for mask computation
        self.total_attributes = total_attributes    #needed for mask computation
    ...
    def forward(self, input):
        ...
        #if  we are dealing with the discriminator we need to turn output back into correct format (add attributes)
        if self.discriminator:
