...
class Discriminator(torch.nn.Module):
    ...
    def __init__(self, input_dim: int, num_layers: int = 5, num_units: int = 200,
        ...
        if self.features:
            seq.append(MaskingLayer(mask_value=1,max_seq_len=self.max_seq_len,total_attributes=self.total_attributes))
...
class MaskingLayer(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        feat = torch.reshape(feat,(input.shape[0],self.max_seq_len,int(feat.shape[1]/self.max_seq_len)))
